...
class PipeablePage {
    ...
    class LoadPageSignal {
        ...
        func signalURLLoadedResult(_ url: String?, withError error: Error? = nil) {
            if error == nil {
                ...
                self.loadCompletion?.resume(returning: ())
                self.loadCompletion = nil
            } else {
                self.isPageLoaded = false
                self.loadCompletion?.resume(throwing: error!)
                self.loadCompletion = nil
            }
    ...
    class Delegate: NSObject, WKNavigationDelegate {
        ...
        func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!) {
            ...
            self.loadPageSignal.signalURLLoadedResult(webView.url?.absoluteString)
        }
        
        func webView(_ webView: WKWebView, didStartProvisionalNavigation navigation: WKNavigation!) {
            print("nav started \(webView.url?.absoluteString ?? "")")
            self.loadPageSignal.isPageLoaded = false
        }
        
        func webView(_ webView: WKWebView, didFail navigation: WKNavigation!, withError error: Error) {
            self.loadPageSignal.signalURLLoadedResult(webView.url?.absoluteString, withError: error)
        }
                ...
                if ctx == "sophia" {
                    ...
                    if let payload = dict["payload"] as? [String: AnyObject] {
                        if name == "frameInfoId" {
                            ...
                            self.frameInfoResolver.synchronizationQueue.sync {
    ...
    init(_ webView: WKWebView, _ frame: WKFrameInfo?) {
        ...
        if frame == nil {
            self.delegate = Delegate(self.loadPageSignal)
            self.webView.navigationDelegate = self.delegate
            
            self.webView.configuration.userContentController.add(ContentController(self.frameInfoResolver), name: "handler")
        }
    }
    ...
    func goto(_ url: String) async throws {
        ...
        self.loadPageSignal.isPageLoaded = false
        await self.webView.load(URLRequest(url: URL(string: url)!))
        
        return try await self.waitForPageLoad()
    }
    ...
    func waitForPageLoad() async throws {
        ...
        if self.loadPageSignal.isPageLoaded {
            return
        }
    ...
    func querySelector(_ selector: String) async throws -> PipeableElement? {
        try await self.waitForPageLoad()
        
        let result = try await webView.callAsyncJavaScript(
            """
                return window.SophiaJS.$(selector);
            """, arguments: ["selector": selector], in: self.frame, contentWorld: WKContentWorld.page)
        print(result!)
    ...
    func xpathSelector(_ xpath: String) async throws -> [PipeableElement] {
        ...
        let result = try await webView.callAsyncJavaScript(
            ...
            """, arguments: ["xpath": xpath], in: self.frame, contentWorld: WKContentWorld.page)
        print(result!)
    ...
    func waitForXPath(_ xpath: String, timeout: Int = 30000, visible: Bool = false) async throws -> PipeableElement? {
        ...
        let result = try await webView.callAsyncJavaScript(
            ...
            """, arguments: ["xpath": xpath, "opts": ["timeout": String(timeout), "visible": true] as [String: Any]], in: self.frame, contentWorld: WKContentWorld.page)
    ...
    func waitForSelector(_ selector: String, timeout: Int = 30000, visible: Bool = false) async throws -> PipeableElement? {
        ...
        let result = try await webView.callAsyncJavaScript(
            ...
            """, arguments: ["selector": selector, "opts": ["timeout": String(timeout), "visible": visible] as [String: Any]], in: self.frame, contentWorld: WKContentWorld.page)
        print(result!)
    ...
    func waitForXHR(_ url: String, timeout: Int = 30000) async throws -> XHRResult? {
        ...
        let result = try await webView.callAsyncJavaScript(
            ...
            """, arguments: ["url": url, "timeout": timeout], in: self.frame, contentWorld: WKContentWorld.page)
    ...
    func evaluate(_ javascript: String) async throws -> String {
        let result = try await self.webView.callAsyncJavaScript(javascript, arguments: [:], in: self.frame, contentWorld: WKContentWorld.page)
...
class PipeableElement {
    ...
    func click() async throws {
        let result = try await self.page.webView.callAsyncJavaScript(
            """
                return window.SophiaJS.click(elementHash);
            """, arguments: ["elementHash": self.elementId], in: self.page.frame, contentWorld: WKContentWorld.page)
        
        print(result!)
    }
    
    func type(_ text: String, _ delay: Int = 10) async throws {
        _ = try await self.page.webView.callAsyncJavaScript(
            """
                return window.SophiaJS.type(elementHash, text, opts);
            """, arguments: ["elementHash": self.elementId, "text": text, "opts": ["delay": delay]], in: self.page.frame, contentWorld: WKContentWorld.page)
    }
    ...
    func contentFrame() async throws -> PipeablePage? {
        ...
        let result = try await self.page.webView.callAsyncJavaScript(
            ...
            """, arguments: ["elementHash": self.elementId, "requestId": requestId], in: self.page.frame, contentWorld: WKContentWorld.page)
    ...
    func getAttribute(_ attributeName: String) async throws -> String? {
        let result = try await self.page.webView.callAsyncJavaScript(
            ...
            """, arguments: ["elementHash": self.elementId, "attributeName": attributeName], in: self.page.frame, contentWorld: WKContentWorld.page)
    ...
    func querySelector(_ selector: String) async throws -> PipeableElement? {
        let result = try await self.page.webView.callAsyncJavaScript(
            ...
            """, arguments: ["selector": selector, "parentHash": self.elementId], in: self.page.frame, contentWorld: WKContentWorld.page)
        
        if let elementId = result as? String {
            return PipeableElement(self.page, elementId)
        }
    ...
    func waitForSelector(_ selector: String, timeout: Int = 30000, visible: Bool = false) async throws -> PipeableElement? {
        let result = try await self.page.webView.callAsyncJavaScript(
            """
                return window.SophiaJS.waitForSelector(selector, opts);
            """, arguments: ["selector": selector, "opts": ["parentElementHash": self.elementId, "timeout": String(timeout), "visible": visible] as [String: Any]], in: self.page.frame, contentWorld: WKContentWorld.page)
        print(result!)
    ...
    func xpathSelector(_ xpath: String) async throws -> [PipeableElement] {
        let result = try await page.webView.callAsyncJavaScript(
            ...
            """, arguments: ["xpath": xpath, "parentHash": self.elementId], in: self.page.frame, contentWorld: WKContentWorld.page)
        print(result!)
    ...
    func waitForXPath(_ xpath: String, timeout: Int = 30000, visible: Bool = false) async throws -> PipeableElement? {
        let result = try await self.page.webView.callAsyncJavaScript(
            """
                return window.SophiaJS.waitForXPath(xpath, opts);
            """, arguments: ["xpath": xpath, "opts": ["timeout": String(timeout), "visible": true, "parentElementHash": self.elementId] as [String: Any]], in: self.page.frame, contentWorld: WKContentWorld.page)
        print(result!)
