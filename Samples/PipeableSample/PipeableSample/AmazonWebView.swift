...
private class WKWebViewController: UIViewController {
    ...
    func start() async throws {
        ...
        try await loginAmazon(page: page, year: 2018)

        onStatusChange(.working, [])

        do {
            orders = try await fetchPurchasesFromAmazon(page: page)
...
struct PipeableWebview_Previews: PreviewProvider {
    ...
    static var previews: some View {
        VStack {
            AmazonWebView(orders: $orders, onClose: {}, onResult: { _ in })
        }
    }
}
